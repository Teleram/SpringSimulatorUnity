...
\begin{abstract}
	\todo[inline]{Die Problematik kurz in 1-2 Saetzen beschreiben}
	\added{Die konvetionelle AI in Spielen hat einen konstaten Schwierigkeitsgrad.
	Fuer die Einsteiger, kann sie zu schwer sein, fuer Experten zu einfach.
Eine neue, lernfaehige AI koennte den Spielspass steigern und die Qualitaet der Spiele erhoehen.}
...
\added{
	Das Projektziel wurde im Laufe des Projekts angepasst aus den folgendne Gruenden.}
\begin{enumerate}
...
Spring hat sich legedlich angeboten, weil es ein OpenSource Projekt ist und theoretisch, wir unseren Code leicht einschleusen koennen.
Die Komplexitaet des Projekt macht es dennoch schwierig es umzusetzen.
Deshalb haben wir haben den Konzept, das wir umsetzen wollten, aus dem urspruenglichen Ziel extrahiert und und auf einer anderen Weise umgesetzt, d.h., statt ihn im Spring umzusetzen haben wir beschlossen unser eigenes Spielfeld zu bauen und dort 
}
...
\added{Dieses Kapitel beschreibt die Entwicklungsumgebung "Unitity" und die in der Arbeit verwendeten Algorithmen.}

\todo[inline]{Unity}
\todo[inline]{Biologiesches Vorbild der Gene. Chomosome und Gene etwas beschreiben.}
\todo[inline]{Neuronale Netze, typischer Trainingsalgorithmen}
\todo[inline]{Genetischer Algorithm. Was unterscheidet ihn von konventionellen Algorithmen. Wie funktioniert das?}
...
Jeder der Spieler hat ein Hauptgebäude \added{(repraesentiert durch grosse, farbige Quadrate in den Spielfeldecken)}, dass in regelmäßigen Zeitabständen Einheiten \added{(kleine, farbige Quadrate)} erstellt \added{(\Cref{Spielfeld})}. 
...
\added{Die Einheiten haben eine Sichtweite ($r_{\text{view}}$) in der sie sowohl die gegnerische als auch freundliche Einheiten sehenn koennen.
Ein kleinerer Radius ($r_{\text{kill}}$)} legt den Bereich fest, indem die Einheiten einander auslöschen können.
\todo[inline]{Wenn alle Einheiten den gleichen kill-Radius haben, wer toetet zuerst?}
...
\todo[inline]{Unity wuerde ich oben beschreiben. Hier geht es eher um die Umsetzung des Konzeptes. 
...
Als Input verwenden wir die eigene Position der Einheit \added{pos.x, pos.y} auf der Ebene, die wir einfach linear reskalieren.
...
\todo[inline]{Also wenn ihr die konkreten Klassen einfuehrt, dann ist es Ratsam ein Klassen Diagramm einzufuehren. So wie es jetzt geschrieben ist, hat wenig Nutzen.}
...
\todo[inline]{Das ist nicht ganz verstaendlich. Koennt ihr ein paar Saetze dazu schreiben, warum es sich um einen natuerlichen Bewertungsstrategie handelt.}
...
(3) Die Ueberschriften sind suboptimal gewaehlt.}
...
\todo[inline]{Namensschema ''1zu1'', ''1zu2'', ''0zu1'' erklaeren.} 
...
\todo[inline]{Grundlage gelegt, fuer die weitere Entwicklung der AI.
Diese Arbeit hilft der naechsten Entwickler nicht in die Fallen zu tappen.}
...
\todo[inline]{Further Work:\\
	...
	Wie wuerdet Ihr vorgehen, wenn Ihr das Projekt neubeginnen wuerdet? Z.B. anderen Algorithmus nehmen, von Scratch beginne, nur auf den Kern konzentrieren, \dots\\
	Was wuerdet Ihr allgemein den naechsten Studenten sagen, die Eure Arbeit fortsetzen?
}
